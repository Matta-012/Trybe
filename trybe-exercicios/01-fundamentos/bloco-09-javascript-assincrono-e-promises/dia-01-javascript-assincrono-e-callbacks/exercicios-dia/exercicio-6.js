...
const sendMarsTemperature = (temperatureCallback, errorCallback) => {
  const message = setTimeout(() => {
    const temperature = getMarsTemperature();

    const isMessageSent = Math.random() >= 0.6;

    const errorReason = 'Robot is busy';

    const curiosity = isMessageSent ? temperatureCallback(temperature) : errorCallback(errorReason)
